...
2. Обновить PAC-скрипт (локальная база блокировок): кликнуть по иконке -> [обновить] (желательно _Оба и на свитчах_)
...
1. Условие: расширение корректно установлено, синяя лента присутствует, всё ещё раз обновлено.
2. Условие: сайт, на котором вы тестируете, имеется в [реестре](http://reestr.rublacklist.net) (см. иконку закрытого замочка).
   Например, протестируйте на  (пока в реестре)
3. Если ошибок нет, но сайт __из реестра__ всё равно не открывается, то нужно разбираться:
